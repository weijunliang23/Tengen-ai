...
- **智能分析**：形势判断（双方子+空与领先方）+ Top-3 候选着法及结构化理由，可点击预览；桌面端启用 KataGo 后切换为**胜率/目差评估**
- **KataGo 引擎（桌面端）**：设置中可动态开关、填引擎/权重路径、调思考量滑条（越强越慢）、测试连接；Web/H5 版自动使用规则 AI
- **局域网对战**：两台设备连接同一局域网服务器自动配对（先连执黑），支持提子/认输/再来一局
...
```bash
npm run server          # 在任一设备启动对战服务器（默认端口 8080）
```

1. 两台设备连接到**同一 Wi-Fi / 局域网**；
2. 一台运行 `npm run server`（终端会显示 `ws://<IP>:8080` 地址）；
3. 两台设备打开应用 → 设置选「**联机对弈**」→ 输入 `ws://<IP>:8080` 点连接；
4. 自动配对：先连执黑、后连执白；对局参数以先连一方为准。

> 联网对战规划见 `docs/online-play.md`（Node + SQLite 轻量方案，未实现）。
...
Web/H5 版只能用规则 AI；**Electron 桌面端**可启用 KataGo 获得强棋力与胜率分析：

1. 下载 KataGo：<https://github.com/lightvector/KataGo/releases>（**RTX 50 系请用 2025 年后版本**）
2. 下载权重（.bin.gz），如 `kata1-b18c384nbt-*.bin.gz`（5070 推荐 b18，约 60MB）
3. 设置 → 勾选「启用 KataGo」→ 填引擎/权重路径 → 「测试连接」
4. 调整「思考量」滑条：低 = 快而弱（陪新手），高 = 慢而强（对战/分析）

- 未配置或启用失败时，自动回退规则 AI；
- 智能分析也会用 KataGo 的胜率/目差（与规则理由混合展示）。
